...
namespace Cursova4
{
    ...
    public partial class FormOdejda : Form
    {
        ...
        {
            dgw.Rows.Add(record.GetInt32(0), record.GetString(1), record.GetInt32(2), record.GetString(3), record.GetString(4), record.GetInt32(5), record.GetString(6), RowState2.ModifiedNew);
        }
        ...
        private void FormOdejda_Load(object sender, EventArgs e)
        {
            CreateColumns1();
        ...
        private void SearchBox(DataGridView dgw)
        {
            ...
            string query = $"Select * from [Одежда] Where concat ([Код товара], Бренд, Цена, Состав, [Тип одежды], [Код поставщика], [Наименование склада]) like '%" + textBox9.Text + "%'";
        ...
        private void updateRows()
        {
            ...
            {
                Debug.WriteLine(ind);
                var rowState = RowState2.A;

                try
                {
                    rowState = (RowState2)dataGridView1.Rows[ind].Cells[7].Value;
                }
                catch
                {
                ...
                if (rowState == RowState2.Deleted)
                {
                    ...
                    var deleteQuery = $"Delete from [Одежда] Where [Код товара] = '{id}';";
                ...
                if (rowState == RowState2.Modified)
                {
                    ...
                    var changeQuery = $"Update [Одежда] Set [Код товара] = '{id1}', Бренд = '{id2}', Цена = '{id3}', Состав = '{id4}', [Тип одежды] = '{id5}', [Код поставщика] = '{id6}', [Наименование склада] = '{id7}' Where [Код товара] = '{id1}'";
        ...
        {
            FormNewOdejda formNewOdejda = new FormNewOdejda();
            formNewOdejda.Show();
        }
    }
}
